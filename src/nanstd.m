...
%
%   NANSTD normalizes Y by N-1 if N>1, where N is the sample size.  This is
...
%
%   Y = NANSTD(X,1) normalizes by N and produces the square root of the second
%   moment of the sample about its mean.  STD(X,0) is the same as STD(X).
%
%   Y = NANSTD(X,W) computes the standard deviation using the weight vector W.
...
%   given weight 1.
%
%   Y = NANSTD(X,W,DIM) takes the standard deviation along the dimension DIM
