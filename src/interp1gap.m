...
ind_int = false(size(vq));  
% For each gap, find corresponding xq indices: 
if x_gap(1)>=0 % assume x is montonically increasing
    for N=1:numel(ind)
        ind_int = ind_int | (xq>x(ind(N)) & xq<x(ind(N)+1));
    end
else      % assume x is monotonically decreasing
    for N=1:numel(ind)
        ind_int = ind_int |(xq>x(ind(N)+1) & xq<x(ind(N)));
    end
end
